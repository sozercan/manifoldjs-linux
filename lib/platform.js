                    ...
                    npm.commands.install(targetTemplateDir, deps, function (er, data) {
                        packager({
                          ...
                          if(!err) {
                            ...
                            if(w3cManifestInfo.content.icons)
                            {
                              var iconLength = w3cManifestInfo.content.icons.length;
                              var iconSrc = w3cManifestInfo.content.icons[iconLength-1].src;
                              iconFileName = iconSrc.substr(iconSrc.lastIndexOf("/") + 1);
                            }

                            var logoPath = platformDir + '/images/'+ iconFileName;
                            var resourcesDir = path.join(platformDir
                            ,'/out/'
                            ,name
                            +'-'
                            ...
                            fs.copy(manifestFilePath, resourcesDir+'manifest.json', function (err) {
                              if (err) return console.error("No W3C Web Manifest found");
                            });

                            fs.copy(logoPath, resourcesDir+iconFileName, function (err) {
                              if (err) return console.error("No suitable icon found");
                            });
                          }
                        });
                    });
